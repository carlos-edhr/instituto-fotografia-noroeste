    ...
    <div className="flex flex-col gap-y-3 max-h-60 overflow-y-auto pr-2">
      {isLoading ? (
        <div className="flex items-center justify-center p-4">
          <LoaderIcon className="size-5 text-red-500 animate-spin" />
        </div>
      ) : (
        data?.pages.map((page) =>
          page.docs.map((tag) => (
            <div
              key={tag.id}
              className={cn(
                "flex items-center justify-between cursor-pointer p-2 rounded-lg transition-colors duration-300",
            ...
            >
              <p className="font-light text-white">{tag.name}</p>
              <Checkbox
      ...
      {hasNextPage && (
        <Button
          ...
          className="font-light text-red-400 hover:text-red-300 mt-2 justify-start p-0 disabled:opacity-50 transition-colors duration-300"
        >
